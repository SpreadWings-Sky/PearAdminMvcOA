...
{
    public class ConTionDataController : ApiController
    {
        [HttpGet]
        public IHttpActionResult ConTiList(int page = 1, int limit = 10)
        {
            ...
            {
                var list = db.Management.OrderBy(p => p.ManId).Skip((page - 1) * limit).Take(limit).Select(p => new
                {
                    ManId = p.ManId,
                    ManRoom = p.ManRoom,
                    ManTitle = p.ManTitle,
                    ...
                    State =p.StateTime>DateTime.Now? "预约中" : "会议中"
                }).ToList();
                var Count = db.Management.Count();
        ...
        public IHttpActionResult ConTiList(int page, int limit, string UserName, string RoomName, string Time)
        {
            ...
            else
            {
                using (OAEntities db = new OAEntities())
                {
                    var list = db.Management.OrderBy(p => p.ManId).Skip((page - 1) * limit).Take(limit);
                    ...
                    if (UserName == null && Time == null && RoomName != null)
                    {
                        ...
                        var Count1 = db.Management.Count();
        ...
        //修改
        [HttpPut]
        public IHttpActionResult ManModify(Management ma)
        {

            using (OAEntities db = new OAEntities())
            {
                Management lits = db.Management.FirstOrDefault(p => p.ManId == ma.ManId);
        ...
        public IHttpActionResult ManDelete(int ManId)
        {
            using (OAEntities db = new OAEntities())
            {
                Management lits = db.Management.FirstOrDefault(p => p.ManId == ManId);
                db.Management.Remove(lits);
                bool stc = false;
                string mes = "删除失败";
                if (db.SaveChanges() > 0)
                {
        ...
        //添加
        [HttpPost]
        public IHttpActionResult ManAdd(Management d)
        {
            using (OAEntities db = new OAEntities())
            {
                db.Management.Add(d);
        ...
        [HttpGet]
        public IHttpActionResult RoomName(string warehouse)
        {
            using (OAEntities db = new OAEntities())
            {
                var list3 = db.ConferenceRoom.Where(p => p.RoomName == p.RoomName);
                return Json(new { msg = "", data = list3.ToList() });
            }
        }
    }
}
